...
def create_backup(path_to_backup: FilesToBackup):
   ...
   files = [f for f in os.listdir(path_to_backup.folder_path) if re.match(path_to_backup.filter_file, f)]
   for file in files:
      shutil.copy2(os.path.join(path_to_backup.folder_path, file), temp_folder.name)
   
   # Then zip the temp folder
   fullpath_output_zip = os.path.join(path_to_backup.folder_path, path_to_backup.zip_name)
